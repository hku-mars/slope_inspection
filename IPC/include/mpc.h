...
class MPCClass {
public:
    typedef std::shared_ptr<MPCClass> Ptr;
    MPCClass(){}
    MPCClass(ros::NodeHandle& nh) {
        std::string node_name = ros::this_node::getName();
        Drag_.setZero();

        nh.param(node_name + "/mpc/horizon", MPC_HORIZON, 5);
        nh.param(node_name + "/mpc/step",    MPC_STEP, 0.1);
        nh.param(node_name + "/mpc/ctrl_delay", ctrl_delay_, 0);
        u_last_.resize(ctrl_delay_);
        for (int i = 0; i < u_last_.size(); i++) {
            u_last_[i] = Eigen::Vector3d::Zero();
        }

        nh.param(node_name + "/mpc/R_p",  R_p_, 100.0);
        nh.param(node_name + "/mpc/R_v",  R_v_, 0.0);
        nh.param(node_name + "/mpc/R_a",  R_a_, 0.0);
        nh.param(node_name + "/mpc/R_u",  R_u_, 10.0);
        nh.param(node_name + "/mpc/R_u_con",  R_u_con_, 1.0);
        nh.param(node_name + "/mpc/R_pN", R_pN_, 0.0);
        nh.param(node_name + "/mpc/R_vN", R_vN_, 0.0);
        nh.param(node_name + "/mpc/R_aN", R_aN_, 0.0);

        nh.param(node_name + "/mpc/D_x", Drag_(0, 0), 0.0);
        nh.param(node_name + "/mpc/D_y", Drag_(1, 1), 0.0);
        nh.param(node_name + "/mpc/D_z", Drag_(2, 2), 0.0);

        nh.param(node_name + "/mpc/vx_min", v_min_.x(), -1.0);
        nh.param(node_name + "/mpc/vy_min", v_min_.y(), -1.0);
        nh.param(node_name + "/mpc/vz_min", v_min_.z(), -1.0);
        nh.param(node_name + "/mpc/vx_max", v_max_.x(),  1.0);
        nh.param(node_name + "/mpc/vy_max", v_max_.y(),  1.0);
        nh.param(node_name + "/mpc/vz_max", v_max_.z(),  1.0);

        nh.param(node_name + "/mpc/ax_min", a_min_.x(), -1.0);
        nh.param(node_name + "/mpc/ay_min", a_min_.y(), -1.0);
        nh.param(node_name + "/mpc/az_min", a_min_.z(), -1.0);
        nh.param(node_name + "/mpc/ax_max", a_max_.x(),  1.0);
        nh.param(node_name + "/mpc/ay_max", a_max_.y(),  1.0);
        nh.param(node_name + "/mpc/az_max", a_max_.z(),  1.0);

        nh.param(node_name + "/mpc/ux_min", u_min_.x(), -1.0);
        nh.param(node_name + "/mpc/uy_min", u_min_.y(), -1.0);
        nh.param(node_name + "/mpc/uz_min", u_min_.z(), -1.0);
        nh.param(node_name + "/mpc/ux_max", u_max_.x(),  1.0);
        nh.param(node_name + "/mpc/uy_max", u_max_.y(),  1.0);
        nh.param(node_name + "/mpc/uz_max", u_max_.z(),  1.0);
    ...
    void SetGoal(Eigen::Vector3d pr, Eigen::Vector3d vr, Eigen::Vector3d ar, int step) {
        ...
        //     ROS_ERROR("ipc set pos error! goal: %f, %f, %f.", pr.x(), pr.y(), pr.z());
        // }
        // if (std::isnan(vr.x()) || std::isnan(vr.y()) || std::isnan(vr.z())) {
        //     ROS_ERROR("ipc set vel error! goal: %f, %f, %f.", vr.x(), vr.y(), vr.z());
        // }
        // if (std::isnan(ar.x()) || std::isnan(ar.y()) || std::isnan(ar.z())) {
        //     ROS_ERROR("ipc set acc error! goal: %f, %f, %f.", pr.x(), pr.y(), pr.z());
        // }
    ...
    void SetStatus(Eigen::Vector3d p0, Eigen::Vector3d v0, Eigen::Vector3d a0) {
        ...
        //     ROS_ERROR("ipc odom error! goal: %f, %f, %f.", p0.x(), p0.y(), p0.z());
        // }
