...
bool MPCClass::Run(void)
{
    ...
    if (init_flag) {
        ...
        std::cout << "sfc: " << std::endl << planes_[0] << std::endl;
        std::cout << "mpc_.Asys: " << mpc_.A.rows() << " " << mpc_.A.cols() << std::endl;
        std::cout << "mpc_.Alow: " << mpc_.Alow.rows() << " " << mpc_.Alow.cols() << std::endl;
        std::cout << "mpc_.Aupp: " << mpc_.Aupp.rows() << " " << mpc_.Aupp.cols() << std::endl;
        std::cout << "mpc_.f: " << mpc_.f.rows() << std::endl;
...
void MPCClass::SetFSC(Eigen::Matrix<double, Eigen::Dynamic, 4>& planes, int step)
{
    if (planes.rows() == 0) {
        ROS_WARN("[MPC]: No sfc!");
        return;
    ...
    static int size0 = 0;
    if (step == 0) size0 = planes.rows();
    if (planes.rows() != size0 && step > 0) {
        std::cout << step << " sfc error! " << std::endl;
        std::cout << step-1 << " sfc: " << std::endl << planes_[step-1] << std::endl;
        std::cout << step << " sfc: " << std::endl << planes << std::endl;
    }
    planes_[step] = planes;
...
void MPCClass::ProblemFormation(void)
{
...
void MPCClass::SystemModel(Eigen::MatrixXd& A, Eigen::MatrixXd& B, double t)
{
    A.resize(9, 9);
    A.setZero();
...
void MPCClass::MPCModel(const Eigen::MatrixXd& A, const Eigen::MatrixXd& B, 
...
void MPCClass::QuadraticTerm(mpc_osqp_t& mpc, const Eigen::MatrixXd& Q, const Eigen::MatrixXd& R, 
...
void MPCClass::LinearTerm(mpc_osqp_t& mpc, const Eigen::VectorXd& x_0, const Eigen::VectorXd& x_r)
{
    if (x_r.rows() != mpc.M.rows()) {
        ROS_ERROR("[MPC]: MPC linear term set goal error!");
        return;
    }
    mpc.f.resize(mpc.C.cols(), 1);
    mpc.f = ((x_0.transpose()*mpc.M.transpose() - x_r.transpose()) * mpc.Q_bar * mpc.C).transpose();
}

void MPCClass::ALLConstraint(mpc_osqp_t& mpc)
{
...
void MPCClass::UpdateBound(mpc_osqp_t& mpc, const Eigen::VectorXd& x_0)
{
